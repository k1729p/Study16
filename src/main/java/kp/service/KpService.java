...
public class KpService {
	...
	public void addBoxesFromFile() throws IOException {
		...
		final List<Box> boxList = new ObjectMapper().readValue(Constants.DATASET_FILE, new TypeReference<List<Box>>() {
		});
	...
	public void searchWithQuery() throws IOException {
		...
		hitList.stream().forEach(hit -> {
			...
			strBld.append(String.format("%n\t%02d, score[%.3f], keyword[%s], project[%s], category[%s]%n"//
					...
					atomic.incrementAndGet(), hit.score(), box.keyword(), box.project(), box.category(), box.path(),
					box.fileName(), box.fileExtension(), box.lineNumber()));
		});
