...
<meta charset="UTF-8">
<body>
<a href="https://github.com/k1729p/Study16/tree/main/docs">
    <img alt="" src="images/ColorScheme.png" height="25" width="800"/></a>
<h2 id="contents">Study16 README Contents</h2>
<h3 id="top">Research the <a href="https://www.elastic.co/">Elasticsearch</a> with the <a
        href="https://www.elastic.co/kibana/">Kibana</a></h3>

<p><img alt="" src="images/MermaidFlowchart.png" height="230" width="500"/><br>
    <img alt="" src="images/blackArrowUp.png">
    <i>The flowchart with <b>Elasticsearch</b> and <b>Kibana</b>.</i>
</p>

<p>Elastic Docs:
</p>
<ul>
    <li><a href="https://www.elastic.co/docs/get-started/the-stack">
        Elastic Stack and its components</a></li>
    <li><a href="https://www.elastic.co/guide/en/elasticsearch/client/java-api-client/current/index.html">
        Java API Client for <b>Elasticsearch</b></a></li>
    <li><a href="https://www.elastic.co/docs/deploy-manage/api-keys/elasticsearch-api-keys">
        <b>Elasticsearch</b> API keys</a></li>
    <li><a href="https://www.elastic.co/docs/explore-analyze/query-filter/tools/console">
        Run API requests with <b>Kibana</b> Console</a></li>
</ul>

<p>The sections of this project:
</p>
<ol>
    <li><a href="#ONE"><b>Elasticsearch</b> and <b>Kibana</b> Launch</a></li>
    <li><a href="#TWO"><b>Elasticsearch</b> Client Queries</a></li>
    <li><a href="#THREE"><b>Kibana</b> Console Queries</a></li>
</ol>

<p>Java source code:<br>
    <img alt="" src="images/aquaHR-500.png"><br>
    <img alt="" src="images/aquaSquare.png"> <a
            href="https://github.com/k1729p/Study16/tree/main/src/main/java/kp">kp</a><br>
...
<p><img alt="" src="images/MermaidClassDiagram.png" height="290" width="190"/><br>
    <img alt="" src="images/blackArrowUp.png">
    <i>The model object
        <a href="https://github.com/k1729p/Study16/blob/main/src/main/java/kp/model/Box.java">
            'Box'</a> class diagram.</i>
</p>
...
<p>Action:<br>
    ...
    <img alt="" src="images/orangeSquare.png"> 3. Take the enrollment token for <b>Kibana</b> from the console log
        (in the <b>Docker</b> 'elasticsearch' container).<br>
    <img alt="" src="images/orangeSquare.png"> 4. Take the password for the 'elastic' user from the console log
        (in the <b>Docker</b> 'elasticsearch' container).<br>
    <img alt="" src="images/orangeSquare.png"> 5. Start <b>Kibana</b> on
    '<a href="http://localhost:5601/app/home#/">http://localhost:5601/app/home#/</a>'.<br>
    <img alt="" src="images/orangeSquare.png"> 6. Log in with user 'elastic' and password for the 'elastic' user.<br>
    <img alt="" src="images/orangeSquare.png"> 7. To make requests to <b>Elasticsearch</b> REST API create user API key.<br>
    <img alt="" src="images/orangeSquare.png"> 8. In <b>Kibana</b> select menu: 'Management' / 'Stack Management' / 'Security' /
        '<a href="http://localhost:5601/app/management/security/api_keys">API keys</a>'.<br>
    <img alt="" src="images/orangeSquare.png"> 9. Create key with name "kp_API_key"
        ("Control security privileges" is on, "Add metadata" is off).<br>
    <img alt="" src="images/orangeSquare.png"> 10. With this user API key replace the 'ELASTICSEARCH_API_KEY' value
        in the file '03 MVN clean install run.bat'.<br>
    <img alt="" src="images/orangeSquare.png"> 11. With the command <i>'cat config/certs/http_ca.crt'</i>
    get the certificate content from the 'elasticsearch' container.<br>
    <img alt="" src="images/orangeSquare.png"> 12. With that certificate replace the content of the certificate file
...
<p><img alt="" src="images/greenCircle.png">
    2.1. The big file <a
            href="https://github.com/k1729p/Study15/blob/main/solr-requests/dataset.json">'<b>dataset.json</b>'</a>
    (about 800 KB)
...
<p> In the method
    <a href="https://github.com/k1729p/Study16/blob/main/src/main/java/kp/Application.java#L41">'Application::main'</a>
...
<ul>
    ...
    <li>adds the <a href="https://github.com/k1729p/Study16/blob/main/src/main/java/kp/model/Box.java">'Box'</a>
        objects from the file '<b>dataset.json</b>'
    </li>
    <li>searches with the query</li>
</ul>

<p>
    The method for creating the <b>Elasticsearch</b> client:
    <a href="https://github.com/k1729p/Study16/blob/main/src/main/java/kp/util/ClientCreator.java#L47">'ClientCreator::createElasticsearchClient'</a>.<br>
    The method for recreating the index:
    <a href="https://github.com/k1729p/Study16/blob/main/src/main/java/kp/util/DatasetManager.java#L38">'DatasetManager::recreateIndex'</a>.<br>
    ...
    <a href="https://github.com/k1729p/Study16/blob/main/src/main/java/kp/util/DatasetManager.java#L59">'DatasetManager::addBoxesFromFile'</a>.<br>
    The method for searching with the query:
    <a href="https://github.com/k1729p/Study16/blob/main/src/main/java/kp/Application.java#L68">'Application::searchWithQuery'</a>.
</p>
...
<table style="border:solid">
    ...
    <tr>
        ...
        <td style="border:solid"><code>GET kp_study/_search<br>
            {
            &nbsp;"query" : {
            &nbsp;&nbsp;"match" : {
            &nbsp;&nbsp;&nbsp;"keyword" : "sonar"
...
<p>Action:<br>
    ...
    <img alt="" src="images/orangeSquare.png"> 2. From the left side menu select the item "Management" / "Dev
    Tools".<br>
    <img alt="" src="images/orangeSquare.png"> 3. Run in the <b>Kibana</b> console the first query and the second query.<br>
    <img alt="" src="images/orangeHR-500.png">
</p>
...
<p><img alt="" src="images/greenCircle.png">
    ...
    <img alt="" src="images/blackArrowUp.png">
    <i>The screenshot of the results fragment in the <b>Kibana</b> console.</i>
</p>

<p><img alt="" src="images/greenCircle.png">
    3.2. The results of the second query.<br>
    The <a href="images/ScreenshotKibanaSecondQuery.png">screenshot</a> of the results in the <b>Kibana</b> console.
</p>

<a href="#top">Back to the top of the page</a>
<hr>
<h2>Data Visualization in <b>Kibana</b></h2>
<table style="border:solid">
    <tbody>
    <tr>
...
<table style="border:solid">
    ...
    <tr>
        ...
        <td style="border:solid">resttestclient</td>
    </tr>
    <tr>
